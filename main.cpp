...
void PrintRef(const int &FnXptRef1, const int &FnXptRef2, const int &FnXptRef3, const int &FnXptRef4, const int &FnYptRef1, const int &FnYptRef2, const int &FnYptRef3, const int &FnYptRef4, ofstream &f_out) {
...
void PrintTrace(vector <int> const &vect2PrintX, vector <int> const &vect2PrintY, const int &k, ofstream &f_out) {
	...
	//Un truc pour clean BESOIN DE CA?
	}else{*/
...
int main() {
	...
	if (f_in.fail()) { //test de l'ouverture du fichier
		...
		if (f_in.fail()) { //test de la lecture du fichier
			...
			PrintRef(XptRef1, XptRef2, XptRef3, XptRef4, YptRef1, YptRef2, YptRef3, YptRef4, f_out);

			vector <vector <int>> vectPosXTr;
			vector <vector <int>> vectPosYTr;
			switch (nbTraces) {
			case 4: 
				vectPosXTr.insert(0, vectPosXTr4);
				vectPosYTr.insert(0, vectPosYTr4);
			case 3:
				vectPosXTr.insert(0, vectPosXTr3);
				vectPosYTr.insert(0, vectPosYTr3);
			case 2:
				vectPosXTr.insert(0, vectPosXTr2);
				vectPosYTr.insert(0, vectPosYTr2);
			case 1:
				vectPosXTr.insert(0, vectPosXTr1);
				vectPosYTr.insert(0, vectPosYTr1);
				break;
			}
			for (int j(0); j < nbTraces; j++) {
				PrintTrace(vectPosXTr(j), vectPosXTr(j), j); //
			}
			f_in.close;
			f_out.close;
		}
